...
def build_molecule_formulation(m, N):
    ...
    m.Idb = m.F - 1
    m.fragments = ["C", "O", "*c1ccco1", "*c1ccc(*)c(*)c1"]
    m.covalences = [4, 2, 1, 3]

    ub = [None, N // 4, None, None]
    ...
    # (B1)
    coef_1 = [2**i for i in [13, 12, 11, 10, 6, 7, 8, 9, 1, 2, 3, 4, 5, 0]]
    for v in range(1, m.N):
        expr = 0.0
        for f in range(m.F):
            expr += coef_1[f] * m.X[0, f]
    ...
    # (B2)
    ub_B3 = 2**12 + 2**6 + 2**1 + 1
    expr = 0.0
    for f in range(m.F):
        expr += coef_1[f] * m.X[0, f]
    m.Con.add(expr <= ub_B3)

    # (B3)
    for v in range(m.N):
        expr = m.X[v, 3] + m.X[v, 13]
        m.Con.add(expr <= 1)

    # (B4)
    for u in range(m.N):
        for v in range(u + 1, m.N):
            expr = m.X[u, 1] + m.X[v, 1] + m.A[u, v]
            m.Con.add(expr <= 2)

    # (B5)
    expr = 0.0
    for v in range(m.N):
        expr += m.X[v, 2] + m.X[v, 3]
    m.Con.add(expr <= 2)
